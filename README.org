...
+ =JULIA_DAEMON_FRESH= (default: =false=), when =true= creates a fresh =:Main= module
  for each client call (clears previous vars). When =false=, works like a REPL
  with persistent variables.
+ =JULIA_DAEMON_AUTOREVISE= (default: =true=), automatically calls =Revise.revise()=
  before each client execution (if Revise is loaded). Set to =false= for faster
  execution if you don't need automatic code reloading.
